...
export const SingleProductSpotlight: React.FC<SingleProductSpotlightProps> = ({
  ...
  return (
    <section className="py-7 sm:py-9 lg:py-11 bg-[#F8F9FA] border-b border-[#E4E4E7] relative overflow-hidden">
      <div className="w-full max-w-[1740px] mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 relative z-10">
        ...
        <div className="bg-white border border-[#DCDCE0] rounded-[2px] p-4 sm:p-5 lg:p-6 xl:p-8 grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-8 xl:gap-10 items-center">
          {/* Left Side: Product Image - High-end Editorial Framing without internal letterbox padding */}
          <div className="lg:col-span-5 relative aspect-[3/4] sm:aspect-[4/5] max-h-[440px] lg:max-h-[480px] rounded-[2px] overflow-hidden bg-[#18181B] border border-[#DCDCE0] group w-full mx-auto shadow-sm">
            <img
              src={productImage}
              alt={product.title}
              loading="lazy"
            ...
            <button
              ...
              className="absolute bottom-3 right-3 bg-white/95 hover:bg-[#18181B] hover:text-white border border-[#DCDCE0] text-[#18181B] px-3 py-1.5 rounded-[2px] backdrop-blur-md transition-colors flex items-center gap-1.5 text-[11px] font-bold uppercase cursor-pointer shadow-sm z-10"
            >
          ...
          <div className="lg:col-span-7 space-y-3.5 sm:space-y-4.5">
            <div>
              <div className="flex items-center gap-2 text-[11px] font-mono font-bold text-[#B45309] uppercase tracking-wider mb-1.5">
                <span>{product.category}</span>
                <span>•</span>
                <span>{product.subcategory}</span>
              </div>

              <h2 className="text-xl sm:text-2xl lg:text-3xl font-black uppercase tracking-tight text-[#18181B] leading-tight">
                {product.title}
              </h2>
            ...
            <p className="text-xs sm:text-[13px] text-[#52525B] leading-relaxed font-medium whitespace-pre-line">
              {product.description}
            </p>

            {/* Key Specs */}
            <div className="grid grid-cols-2 gap-2.5 text-xs bg-[#F8F9FA] p-2.5 sm:p-3 rounded-[2px] border border-[#DCDCE0]">
              <div>
                <span className="text-[#71717A] text-[9.5px] uppercase font-bold block">Construção</span>
                <span className="font-bold text-[#18181B]">
                  {product.category === 'jaquetas' ? 'Mangas Contrastantes & Punhos Listrados' : '400g/m² Heavyweight'}
                </span>
              </div>
              <div>
                <span className="text-[#71717A] text-[9.5px] uppercase font-bold block">Modelagem</span>
                <span className="font-bold text-[#18181B]">
            ...
            <div className="flex items-baseline flex-wrap gap-2 sm:gap-3">
              ...
              {product.promoPrice && (
                <span className="text-xs font-bold text-[#71717A] line-through">
                  R$ {product.price.toFixed(2).replace('.', ',')}
                </span>
              )}
              <span className="text-[11px] text-[#B45309] font-mono font-bold">PIX ou cartão na InfinitePay</span>
            </div>

            {/* Color Selector */}
            {product.colors && product.colors.length > 0 && (
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase text-[#52525B] block">
                  ...
                  {product.colors.map((c) => {
                    ...
                    return (
                      <button
                        ...
                        className={`h-8 px-2.5 rounded-[2px] text-[11px] font-bold uppercase transition-all flex items-center gap-1.5 border cursor-pointer ${
                          isSelected
              ...
              <div className="flex flex-wrap gap-2">
                {(product.sizes || ['P', 'M', 'G', 'GG', 'XG']).map((sz) => (
                  <button
                    ...
                    className={`w-9.5 h-9.5 rounded-[2px] text-xs font-bold uppercase transition-colors border cursor-pointer ${
                      selectedSize === sz
                        ? 'bg-[#18181B] text-white border-[#18181B] shadow-sm font-black'
                        : 'bg-[#F4F4F5] text-[#52525B] border-[#E4E4E7] hover:border-[#18181B] hover:text-[#18181B]'
                    }`}
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 pt-1">
              <button
                ...
                className={`flex-1 py-3.5 px-5 rounded-[2px] font-black text-xs uppercase tracking-widest transition-colors flex items-center justify-center gap-2 cursor-pointer ${
                  added
              ...
              <button
                ...
                className="py-3.5 px-5 rounded-[2px] bg-white border border-[#DCDCE0] text-[#18181B] hover:bg-[#F4F4F5] hover:border-[#18181B] font-bold text-xs uppercase tracking-wider transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
              >
