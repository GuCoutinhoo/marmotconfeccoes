...
export const AdminBannersTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Top Controls */}
      <div className="flex items-center justify-between bg-white border border-[#E5E5E1] p-4 rounded-2xl shadow-xs">
        <div>
          <h3 className="text-sm font-black uppercase text-[#171717] tracking-tight flex items-center gap-2">
            <ImageIcon className="w-4 h-4 text-[#B45309]" /> Banners Promocionais & Hero
          </h3>
          <p className="text-xs text-[#6B6B66] mt-0.5">Gerencie os destaques visuais exibidos na vitrine da loja</p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={() => handleOpenModal()}
            className="px-4 py-2 bg-[#F0C84B] hover:bg-amber-400 text-black text-xs font-extrabold uppercase rounded-xl transition-all shadow-xs flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {banners.map((b) => (
          <div
            key={b.id}
            className="bg-white border border-[#E5E5E1] hover:border-[#B45309] rounded-2xl overflow-hidden shadow-xs flex flex-col group transition-all"
          >
            {/* Banner Preview Image */}
            <div className="relative h-48 bg-[#F9F9F7] overflow-hidden">
              <img
                src={b.imageUrl}
                alt={b.title}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-4">
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#F0C84B] bg-black/60 px-2 py-0.5 rounded w-fit mb-1 border border-[#F0C84B]/30">
                  {b.placement} • Ordem {b.order}
                </span>
                <h4 className="text-lg font-black uppercase text-white tracking-tight">{b.title}</h4>
                {b.subtitle && <p className="text-xs text-zinc-300">{b.subtitle}</p>}
              </div>
            ...
            <div className="p-4 flex items-center justify-between gap-3 text-xs">
              <div className="text-[11px] font-mono text-[#6B6B66] truncate">
                Destino: <span className="text-[#171717] font-bold">{b.linkUrl}</span>
              </div>

              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleOpenModal(b)}
                  className="p-2 bg-[#F9F9F7] hover:bg-white border border-[#E5E5E1] rounded-xl text-[#171717] transition-all shadow-xs"
                  title="Editar Banner"
                >
                ...
                <button
                  ...
                  className="p-2 bg-[#F9F9F7] hover:bg-red-50 border border-[#E5E5E1] hover:border-red-200 rounded-xl text-red-600 transition-all shadow-xs"
        ...
        {banners.length === 0 && (
          <div className="col-span-2 p-12 text-center text-[#6B6B66] font-mono text-xs bg-white border border-[#E5E5E1] rounded-2xl shadow-xs">
          ...
          <div className="bg-white border border-[#E5E5E1] rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-xl animate-in zoom-in-95 duration-150">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-[#6B6B66] mt-0.5">Defina as informações e o link de redirecionamento</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="text-[#6B6B66] hover:text-[#171717]">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs font-mono text-[#6B6B66] block mb-1">Título do Banner *:</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-mono text-[#6B6B66] block mb-1">Subtítulo (Opcional):</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-mono text-[#6B6B66] block mb-1">URL da Imagem *:</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-mono text-[#6B6B66] block mb-1">Posição / Local:</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-mono text-[#6B6B66] block mb-1">Ordem de Exibição:</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="text-xs font-mono text-[#6B6B66] block mb-1">Link de Destino:</label>
                <input
                  type="text"
              ...
              <label className="flex items-center gap-2 cursor-pointer pt-1">
                ...
                />
                <span className="text-xs text-[#171717] font-bold">Banner Ativo (Visível na loja)</span>
              </label>
            </div>
