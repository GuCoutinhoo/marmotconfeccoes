    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {categories.map((cat, index) => (
          <div
            key={cat.id || cat.slug}
            className="bg-[#161616] border border-[#262626] hover:border-[#333333] rounded-2xl overflow-hidden flex flex-col justify-between group transition-all shadow-md"
          >
            {/* Category Image Header with overlay actions */}
            <div className="relative h-48 w-full bg-[#080808] overflow-hidden">
              <img
                src={cat.image}
                alt={cat.name}
              ...
              <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-[#080808]/90 border border-[#262626] px-2.5 py-1 rounded-full text-[10px] font-mono font-bold text-[#D6B35A]">
                <span>#{index + 1}</span>
              ...
              <div className="absolute top-3 right-3 flex items-center gap-1 bg-[#080808]/90 border border-[#262626] p-1 rounded-lg">
                <button
                  disabled={index === 0}
                  onClick={() => handleMove(index, 'up')}
                  title="Mover para cima"
                  className="p-1 text-[#777777] hover:text-[#D6B35A] disabled:opacity-30 disabled:hover:text-[#777777]"
                >
              ...
              <button
                ...
                className="absolute bottom-3 right-3 bg-[#080808]/90 hover:bg-[#D6B35A] text-[#EFECE6] hover:text-black border border-[#262626] hover:border-[#D6B35A] px-2.5 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all flex items-center gap-1.5 backdrop-blur-sm"
              >
                <Upload className="w-3 h-3" /> Trocar Foto
              </button>

              <div className="absolute bottom-3 left-3">
                <span className="text-[10px] font-mono font-bold text-[#D6B35A] uppercase tracking-wider">
                  {cat.productCount ? `${cat.productCount} Peças` : 'Catálogo Ativo'}
                </span>
                <h3 className="text-lg font-black uppercase text-[#EFECE6] leading-tight drop-shadow">
                  {cat.name}
                </h3>
              </div>
            </div>
            ...
            <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
              <div>
                <p className="text-xs font-semibold text-[#D6B35A] line-clamp-1">
                  {cat.tagline || 'Sem subtítulo cadastrado'}
                </p>
                <p className="text-[11px] text-[#777777] mt-1 line-clamp-2 leading-relaxed">
                ...
                <div className="flex flex-wrap gap-1 mt-3">
                  {(cat.subcategories || []).slice(0, 3).map((sub, i) => (
                    <span
                      key={i}
                      className="text-[9px] font-medium bg-[#080808] text-[#999999] px-2 py-0.5 rounded border border-[#262626]"
                    >
                      {sub}
                    </span>
                  ))}
                  {(cat.subcategories || []).length > 3 && (
                    <span className="text-[9px] font-medium bg-[#080808] text-[#D6B35A] px-1.5 py-0.5 rounded border border-[#262626]">
                      +{(cat.subcategories || []).length - 3}
                    </span>
                  )}
                </div>
              ...
              <div className="pt-3 border-t border-[#262626] flex items-center justify-between gap-2">
                <button
                  ...
                  className="flex-1 bg-[#080808] hover:bg-[#262626] text-[#EFECE6] border border-[#262626] hover:border-[#D6B35A] py-2 rounded text-xs font-bold uppercase transition-all flex items-center justify-center gap-1.5"
                >
                  <Edit2 className="w-3.5 h-3.5 text-[#D6B35A]" /> Editar
                </button>

                <button
                  onClick={() => setDeletingId(cat.id)}
                  className="p-2 bg-[#080808] hover:bg-red-950/40 text-[#777777] hover:text-red-400 border border-[#262626] hover:border-red-500/40 rounded transition-all"
      ...
      {quickImageCat && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[#161616] border border-[#262626] p-6 sm:p-8 rounded-2xl max-w-lg w-full space-y-6">
            <div className="flex items-center justify-between border-b border-[#262626] pb-4">
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-full bg-[#D6B35A]/10 text-[#D6B35A] flex items-center justify-center border border-[#D6B35A]/30">
                  <ImageIcon className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-base font-black uppercase text-[#EFECE6]">
                    Trocar Foto: {quickImageCat.name}
                  </h3>
                  <p className="text-xs text-[#777777]">Atualize a foto exibida na seção Compre por Categoria</p>
                </div>
              </div>
              <button
            ...
            {/* Live Image Preview */}
            <div className="relative h-56 rounded-xl overflow-hidden border border-[#262626] bg-[#080808]">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  className="bg-[#080808] hover:bg-[#222222] border border-[#262626] hover:border-[#D6B35A] text-[#EFECE6] py-2.5 px-3 rounded-xl text-xs font-bold uppercase transition-all flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="bg-[#1C180F] hover:bg-[#262013] border border-[#D6B35A]/60 text-[#D6B35A] py-2.5 px-3 rounded-xl text-xs font-bold uppercase transition-all flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  ...
                  className="bg-[#080808] hover:bg-[#222222] border border-[#262626] text-[#EFECE6] py-2.5 px-3 rounded-xl text-xs font-bold uppercase transition-all flex items-center justify-center gap-1.5"
                >
                  <ImageIcon className="w-3.5 h-3.5 text-[#777777]" />
                  Inserir URL
                </button>
              </div>

              <div>
                <label className="text-[11px] font-bold text-[#777777] block mb-1">Ou edite a URL diretamente:</label>
                <input
                  type="text"
                  value={quickImageUrl}
                  onChange={(e) => {
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="button"
                onClick={() => setQuickImageCat(null)}
                className="flex-1 bg-[#080808] border border-[#262626] text-[#777777] hover:text-[#EFECE6] py-3 rounded-xl text-xs font-bold uppercase"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 bg-[#D6B35A] text-black font-extrabold py-3 rounded-xl text-xs uppercase hover:bg-[#EFECE6] transition-colors"
              >
                Salvar Nova Foto
              </button>
            </div>
      ...
      {(editingCategory || isAddOpen) && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-2.5 sm:p-4 md:p-6 bg-black/85 backdrop-blur-sm animate-fadeIn overflow-hidden">
          <div className="bg-[#161616] border border-[#262626] rounded-2xl max-w-2xl w-full max-h-[92vh] sm:max-h-[88vh] flex flex-col shadow-2xl overflow-hidden min-w-0 box-border">
            <div className="flex items-center justify-between border-b border-[#262626] px-5 py-4 sm:px-8 sm:py-5 shrink-0 bg-[#161616]">
              <div>
                ...
                <h3 className="text-lg font-black uppercase text-[#EFECE6] truncate">
            ...
            <form onSubmit={handleSaveCategory} className="flex flex-col flex-1 overflow-hidden min-h-0 min-w-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[11px] font-bold text-[#777777] block mb-1">Nome da Categoria *</label>
                  <input
                    type="text"
                    value={formName}
                ...
                <div>
                  <label className="text-[11px] font-bold text-[#777777] block mb-1">Slug (Identificador URL) *</label>
                  <input
                    type="text"
                    value={formSlug}
              ...
              <div>
                <label className="text-[11px] font-bold text-[#777777] block mb-1">Subtítulo / Tagline de Destaque</label>
                <input
                  type="text"
                  value={formTagline}
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-[11px] font-bold text-[#777777] block">Imagem da Categoria</label>
                  <span className="text-[10px] text-[#D6B35A] font-mono">Formato Quadrado (1:1) recomendado</span>
                </div>
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                  <div
                    ...
                    className="group relative w-24 h-24 rounded-xl overflow-hidden border border-[#262626] bg-[#080808] shrink-0 cursor-pointer shadow-md hover:border-[#D6B35A] transition-all"
                  ...
                  >
                    ...
                    />
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center text-[10px] font-bold text-[#D6B35A] transition-opacity">
                  ...
                  <div className="flex-1 w-full space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        disabled={isUploading}
                        className="bg-[#080808] hover:bg-[#222222] border border-[#262626] hover:border-[#D6B35A] text-[#EFECE6] px-3.5 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors"
                      >
                        <Upload className="w-3.5 h-3.5 text-[#D6B35A]" />
                      ...
                      <button
                        ...
                        className="bg-[#1C180F] hover:bg-[#262013] border border-[#D6B35A]/50 text-[#D6B35A] px-3.5 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm"
                      >
                        <Crop className="w-3.5 h-3.5" />
                        Ajustar / Enquadrar
                      </button>
                    ...
                    <input
                      ...
                      className="w-full bg-[#080808] border border-[#262626] px-3.5 py-2 rounded text-xs text-[#EFECE6] focus:outline-none focus:border-[#D6B35A]"
                    />
                  </div>
                </div>
              </div>
              ...
              {/* Subcategories */}
              <div>
                <label className="text-[11px] font-bold text-[#777777] block mb-1">Subcategorias (Filtros)</label>
                <div className="flex gap-2 mb-2">
                  ...
                  <button
                    ...
                    className="bg-[#080808] hover:bg-[#262626] text-[#D6B35A] border border-[#262626] px-3 py-2 rounded text-xs font-bold uppercase"
                  >
                    Adicionar
                  </button>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {formSubcategories.map((sub, idx) => (
                    <span
                      key={idx}
                      className="bg-[#080808] text-[#EFECE6] border border-[#262626] text-xs px-2.5 py-1 rounded-full flex items-center gap-1.5"
                    >
                      {sub}
                      <X
                        className="w-3 h-3 text-[#777777] hover:text-red-400 cursor-pointer"
              ...
              {/* Product count */}
              <div className="w-36">
                <label className="text-[11px] font-bold text-[#777777] block mb-1">Qtd. de Peças</label>
                <input
                  type="number"
                  min={0}
      ...
      {deletingId && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[#161616] border border-[#262626] p-6 rounded-2xl max-w-md w-full space-y-4 text-center">
            ...
            <h3 className="text-base font-black uppercase text-[#EFECE6]">Excluir Categoria?</h3>
