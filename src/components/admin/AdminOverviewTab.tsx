    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#141414] border border-[#222222] p-4 sm:p-5 rounded-2xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#D6B35A]/10 border border-[#D6B35A]/30 flex items-center justify-center text-[#D6B35A]">
            <Activity className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-base font-black uppercase text-[#EFECE6] tracking-tight">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 bg-[#080808] hover:bg-[#1a1a1a] border border-[#262626] rounded-xl text-[#777777] hover:text-[#EFECE6] transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Faturamento Hoje */}
        <div className="bg-[#141414] border border-[#222222] hover:border-[#333333] p-5 rounded-2xl space-y-3 transition-all relative overflow-hidden group">
          ...
          <div>
            <p className="text-2xl font-black text-[#EFECE6] tracking-tight">
              R$ {(metrics?.revenueToday ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </p>
            <p className="text-[11px] text-[#777777] mt-1 flex items-center gap-1.5">
              <Clock className="w-3.5 h-3.5 text-[#D6B35A]" />
              <span className="font-mono text-[#EFECE6] font-bold">{metrics?.ordersToday ?? 0}</span> pedidos registrados hoje
            </p>
          </div>
        ...
        <div className="bg-[#141414] border border-[#222222] hover:border-[#333333] p-5 rounded-2xl space-y-3 transition-all relative overflow-hidden group">
          <div className="flex items-center justify-between">
            ...
            <div className="w-8 h-8 rounded-lg bg-[#D6B35A]/10 border border-[#D6B35A]/20 flex items-center justify-center text-[#D6B35A]">
              <TrendingUp className="w-4 h-4" />
            </div>
          </div>
          <div>
            <p className="text-2xl font-black text-[#D6B35A] tracking-tight">
          ...
          <div className="h-1 w-full bg-[#222] rounded-full overflow-hidden">
            <div className="h-full bg-[#D6B35A] rounded-full w-4/5" />
          </div>
        </div>
        ...
        <div
          ...
          className="cursor-pointer bg-[#141414] border border-[#222222] hover:border-[#D6B35A]/50 p-5 rounded-2xl space-y-3 transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-mono font-bold uppercase tracking-wider text-[#777777]">
              Aguardando Envio
        ...
        <div
          ...
          className="cursor-pointer bg-[#141414] border border-[#222222] hover:border-red-800/60 p-5 rounded-2xl space-y-3 transition-all group"
        >
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-mono font-bold uppercase tracking-wider text-[#777777]">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#777777] mt-1 flex items-center gap-1">
              Estoque baixo: <span className="font-mono text-amber-400 font-bold">{metrics?.lowStockCount ?? 0} itens</span>
            </p>
          </div>
          <div className="h-1 w-full bg-[#222] rounded-full overflow-hidden">
      ...
      <div className="bg-[#141414] border border-[#222222] p-6 rounded-2xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#222222] pb-5">
          ...
          <div className="flex items-center gap-4 text-xs font-mono">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-[#D6B35A]" />
              <span className="text-[#A0A0A0]">Faturamento (R$)</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative w-full overflow-x-auto">
          {salesData.length === 0 ? (
            <div className="h-48 flex items-center justify-center text-xs text-[#777777] font-mono">
            ...
            <div className="min-w-[600px] relative">
              ...
              >
                <defs>
                  <linearGradient id="salesGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#D6B35A" stopOpacity="0.35" />
                ...
                {points.map((pt, idx) => (
                  <g key={idx} className="cursor-pointer">
                    <circle
                      ...
                      stroke="#D6B35A"
                      strokeWidth="2"
                      className="transition-all hover:r-6 hover:fill-[#D6B35A]"
              ...
              {hoveredPoint && (
                <div
                  className="absolute bg-[#1c1c1c] border border-[#D6B35A]/50 p-2.5 rounded-xl text-xs space-y-1 shadow-2xl pointer-events-none z-10 font-mono"
                  style={{
                ...
                >
                  <p className="text-[#888888] text-[10px] uppercase font-bold">{hoveredPoint.date}</p>
                  <p className="text-[#D6B35A] font-black text-sm">
                    R$ {hoveredPoint.revenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                  <p className="text-[#EFECE6] text-[10px]">{hoveredPoint.orders} pedido(s)</p>
                </div>
              )}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Status Distribution */}
        <div className="bg-[#141414] border border-[#222222] p-6 rounded-2xl space-y-5">
          <div className="flex items-center justify-between border-b border-[#222222] pb-4">
            <div>
              <h3 className="text-sm font-black uppercase text-[#EFECE6] tracking-tight flex items-center gap-2">
                <Layers className="w-4 h-4 text-[#D6B35A]" />
                Distribuição por Status
              </h3>
              <p className="text-xs text-[#777777]">Visão operacional do fluxo de pedidos</p>
            </div>
            <button
              onClick={() => onNavigateTab('orders')}
              className="text-xs font-bold text-[#D6B35A] hover:underline flex items-center gap-1"
            >
          ...
          <div className="space-y-3">
            {(metrics?.ordersByStatus || []).map((item, idx) => (
              <div
                key={idx}
                onClick={() => onNavigateTab('orders')}
                className="flex items-center justify-between p-3 rounded-xl bg-[#0a0a0a] border border-[#1f1f1f] hover:border-[#333] transition-all cursor-pointer"
              >
                <div className="flex items-center gap-3">
                  <div
                  ...
                  />
                  <span className="text-xs font-bold text-[#EFECE6]">{item.status}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs font-bold text-[#D6B35A] bg-[#141414] px-2.5 py-1 rounded-lg border border-[#262626]">
                    {item.count}
                  </span>
                </div>
              </div>
            ...
            {(!metrics?.ordersByStatus || metrics.ordersByStatus.length === 0) && (
              <p className="text-xs text-[#777777] font-mono text-center py-6">
        ...
        <div className="bg-[#141414] border border-[#222222] p-6 rounded-2xl space-y-5">
          <div className="flex items-center justify-between border-b border-[#222222] pb-4">
            <div>
              <h3 className="text-sm font-black uppercase text-[#EFECE6] tracking-tight flex items-center gap-2">
                <ShoppingBag className="w-4 h-4 text-[#D6B35A]" />
                Produtos Mais Vendidos
              </h3>
              <p className="text-xs text-[#777777]">Itens com maior tração no período</p>
            </div>
            <button
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-[#EFECE6] truncate">{prod.title}</p>
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-black font-mono text-[#D6B35A]">
                    R$ {prod.revenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </p>
                  <p className="text-[10px] text-[#888888] font-mono">
                    Estoque: <span className={prod.stock < 10 ? 'text-amber-400 font-bold' : 'text-[#888888]'}>{prod.stock}</span>
                  </p>
                </div>
              </div>
            ))}
      ...
      <div className="bg-gradient-to-r from-[#141414] via-[#181818] to-[#141414] border border-[#262626] p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h4 className="text-sm font-black uppercase text-[#EFECE6] tracking-tight">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-[#080808] hover:bg-[#222] border border-[#333] text-[#EFECE6] text-xs font-bold uppercase rounded-xl transition-all flex items-center gap-2"
          >
            <Truck className="w-3.5 h-3.5 text-[#D6B35A]" /> Expedição & Fretes
          </button>
          <button
            onClick={() => onNavigateTab('inventory')}
            className="px-4 py-2.5 bg-[#080808] hover:bg-[#222] border border-[#333] text-[#EFECE6] text-xs font-bold uppercase rounded-xl transition-all flex items-center gap-2"
          >
            <Package className="w-3.5 h-3.5 text-[#D6B35A]" /> Ajustar Estoque
          </button>
          <button
            onClick={() => onNavigateTab('reports')}
            className="px-4 py-2.5 bg-[#D6B35A] hover:bg-[#EFECE6] text-black text-xs font-black uppercase rounded-xl transition-all shadow-md flex items-center gap-2"
          >
